...
class Almoxarifado extends CI_Controller {
    ...
    function __construct() {
		...
		$this->load->model('almox_model','',TRUE);
		$this->data['menuAlmox'] = 'Almox';
	}	
	...
	function gerenciar(){
        ...
        $config['total_rows'] = $this->almox_model->count('os');
        $config['per_page'] = 10;
		...
		$this->data['results'] = $this->almox_model->get('almoxarifado','*','',$config['per_page'],$this->uri->segment(3));
    ...
    function adicionar(){
        ...
        if ($this->form_validation->run('almoxarifado') == false) {
           $this->data['custom_error'] = (validation_errors() ? true : false);
            ...
            $dataAlmox = $this->input->post('dataAlmox');

            try {
                
                $dataAlmox = explode('/', $dataAlmox);
                $dataAlmox = $dataAmox[2].'-'.$dataAlmox[1].'-'.$dataAlmox[0];


            } catch (Exception $e) {
               $dataAlmox = date('Y/m/d'); 
            }
            ...
            if (is_numeric($id = $this->almox_model->add('almoxarifado', $data, true)) ) {
                $this->session->set_flashdata('success','Pedido iniciada com sucesso, adicione os itens.');
                redirect('almoxarifado/editar/'.$id);
        ...
        $this->data['view'] = 'almoxarifado/adicionarAlmox';
        $this->load->view('tema/topo', $this->data);
    }
    ...
    function editar() {
        ...
        $this->data['result'] = $this->almox_model->getById($this->uri->segment(3));
        $this->data['produtos'] = $this->almox_model->getProdutos($this->uri->segment(3));
        $this->data['view'] = 'almoxarifado/editarAlmox';
        $this->load->view('tema/topo', $this->data);
    ...
    public function visualizar(){
        ...
        $this->data['view'] = 'almoxarifado/visualizarAlmox';
        $this->load->view('tema/topo', $this->data);
    ...
    function excluir(){
        ...
        $this->db->delete('itens_de_almox');

        $this->db->where('idAlmox', $id);
        $this->db->delete('almoxarifado');           

        $this->session->set_flashdata('success','Excluída com sucesso!');            
        redirect(base_url().'index.php/almoxarifado/gerenciar/');
    ...
    public function autoCompleteProduto(){
        ...
        if (isset($_GET['term'])){
            ...
            $this->almox_model->autoCompleteProduto($q);
        }
    ...
    public function autoCompleteCliente(){
        ...
        if (isset($_GET['term'])){
            $q = strtolower($_GET['term']);
            $this->almox_model->autoCompleteCliente($q);
        }
    ...
    public function autoCompleteUsuario(){
        ...
        if (isset($_GET['term'])){
            $q = strtolower($_GET['term']);
            $this->almox_model->autoCompleteUsuario($q);
        }
    ...
    public function adicionarProduto(){
        ...
        if($this->form_validation->run() == false){
           echo json_encode(array('result'=> false)); 
            ...
            if($this->almox_model->add('itens_de_almox', $data) == true){
    ...
    function excluirProduto(){
            ...
            if($this->vendas_model->delete('itens_de_almox','idItens',$ID) == true){
